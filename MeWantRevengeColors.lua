...
function Colors:GetColor(Branch, Name)
	if Branch == "Class" then
		if not MeWantRevenge.db.profile.Colors[Branch][Name] then
	...
	if not MeWantRevenge.db.profile.Colors[Branch][Name].a then
		MeWantRevenge.db.profile.Colors[Branch][Name].a = 1
	end
	return MeWantRevenge.db.profile.Colors[Branch][Name]
end

local LastSet

function Colors:SetColor(Branch, Name, c)
	if type(MeWantRevenge.db.profile.Colors[Branch][Name]) ~= "table" then
		MeWantRevenge.db.profile.Colors[Branch][Name] = {}
	end
	MeWantRevenge.db.profile.Colors[Branch][Name].r = c.r
	MeWantRevenge.db.profile.Colors[Branch][Name].g = c.g
	MeWantRevenge.db.profile.Colors[Branch][Name].b = c.b

	MeWantRevenge.db.profile.Colors[Branch][Name].a = c.a
...
function Colors:Debug()
	for k1, Branch in pairs(ItemsToUpdate) do
		for k2, Name in pairs(Branch) do
			MeWantRevenge:Print(getn(Name).." "..k1.." "..k2)

			local Items = ItemsToUpdate[k1][k2]

			for k, v in pairs(TypeToUpdate[k1][k2]) do
				if v == TYPE_TEXTURE then
					MeWantRevenge:Print("Texture:" .. getn(Items[k]))
				elseif v == TYPE_BORDER then
					MeWantRevenge:Print("Border:" .. getn(Items[k]))
				elseif v == TYPE_BACKGROUND then
					MeWantRevenge:Print("Background:" .. getn(Items[k]))
				elseif v == TYPE_FUNC then
					MeWantRevenge:Print("Func:" .. getn(Items[k]))
				elseif v == TYPE_FONT then
					MeWantRevenge:Print("Font:" .. getn(Items[k]))
				end
			end
		end
	end
